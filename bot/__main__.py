...
def stats(update, context):
    ...
    stats = f'<b>⚙️Commit Date:</b> {last_commit}\n\n'\
            ...
            f'<b>💽DISK:</b> {disk}%\n\n'\
            f'<b>⚙️Physical Cores:</b> {cpu_count(logical=False)}\n'\
            f'<b>⚙️Total Cores:</b> {cpu_count(logical=True)}\n\n'\
            f'<b>〽️SWAP:</b> {get_readable_file_size(swap.total)} | <b>📊Used:</b> {swap.percent}%\n'\
...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("Website", "https://bujel.net")
    buttons.buildbutton("Owner", "https://t.me/itsme_rizxx")
